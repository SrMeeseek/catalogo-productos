        ...
        <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" id="name" name="name" class="form-control @error('name') is-invalid @enderror"
                       value="{{ old('name', $product->name) }}" maxlength="150" required>
            ...
            <div class="mb-3">
                ...
                <textarea id="observations" name="observations" rows="4" maxlength="255"
                          class="form-control @error('observations') is-invalid @enderror" required>{{ old('observations', $product->observations) }}</textarea>
            ...
            <div class="mb-3">
                ...
                @if ($product->image)
                    <div class="mb-2">
                        <img src="{{ Storage::url($product->image) }}" alt="Actual" style="height:80px;border-radius:4px;">
                    </div>
                @endif
                <input type="file" id="image" name="image" class="form-control @error('image') is-invalid @enderror"
                       accept="image/*">
                @error('image') <div class="invalid-feedback">{{ $message }}</div> @enderror
                <div class="form-text">Opcional. Subir una nueva imagen reemplaza la actual. Máximo 2MB.</div>
            </div>
